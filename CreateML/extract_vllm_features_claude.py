...
def calculate_incongruence(claude_description, transcript, bert_model, bert_tokenizer, device):
    """
    Calculate incongruence between Claude's thumbnail description and actual video transcript.

    Args:
        claude_description (str): Description generated by Claude.
        transcript (str): Actual video transcript.
        bert_model: BERT model for embeddings.
        bert_tokenizer: BERT tokenizer.
        device: Torch device.

    Returns:
        tuple: (incongruence_score, tvllm_embedding)
            - incongruence_score (float): 0-1, higher = more mismatch
            - tvllm_embedding (np.ndarray): 768-dim BERT embedding of Claude description
    """
    # Encode Claude description
    inputs_desc = bert_tokenizer(
        claude_description,
        return_tensors="pt",
        truncation=True,
        max_length=512,
    ...
    # Encode transcript (truncate to first 5000 chars for efficiency)
    transcript_truncated = transcript[:5000] if len(transcript) > 5000 else transcript
    ...
    inputs_trans = {k: v.to(device) for k, v in inputs_trans.items()}

    with torch.no_grad():
        trans_output = bert_model(**inputs_trans)
        trans_embedding = trans_output.last_hidden_state.mean(dim=1).squeeze()

    # Calculate cosine similarity
    similarity = torch.nn.functional.cosine_similarity(
        tvllm_embedding.unsqueeze(0),
        trans_embedding.unsqueeze(0)
    ).item()

    # Incongruence = 1 - similarity (higher = more clickbait)
    incongruence_score = 1 - similarity
